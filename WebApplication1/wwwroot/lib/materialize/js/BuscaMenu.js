...
$(document).ready(function () {
  ...
  $.ajax({
    type: "POST",
      url: site + "/Sistema/Perfil/BuscaMenu",
    data: {
      ajax: true,
      PerfilID: 1
    },
    success: function (ret) {
      $.each(ret, function (index, value) {
        window.index.add({
          href: value.area + "/" + value.controller,
          title: value.titulo,
          body: value.controller + ", " + value.titulo
        });
      });
    }
  });
  ...
  var renderResults = function (results) {
    ...
    Array.prototype.forEach.call(results, function (result) {
      var resultDiv = $('<p><a href=' + result[1] + '>' + result[0] + '</a></p>');
  ...
  $('input#search').on('keyup', debounce(function (e) {
    ...
    var results = window.index.search(query).slice(0, 6).map(function (result) {
      var href = result.ref.replace("Sistema/", "");
